...
bool ImGui::CustomCheckbox(const char* label, bool* v)
{
    ...
    ImU32 color_bg = *v ? IM_COL32(0, 200, 0, 255) : IM_COL32(200, 0, 0, 255);
    const char* label_text = *v ? "ON" : "OFF";

    // Background (flat color, no border)
    //draw_list->AddRectFilled(p, ImVec2(p.x + width, p.y + height), color_bg, 4.0f); // rounded corners

    // Centered white text
    ImVec2 text_size = ImGui::CalcTextSize(label_text);
    ImVec2 text_pos = ImVec2(
        p.x + (width - text_size.x) * 0.5f,
        p.y + (height - text_size.y) * 0.5f
    );
    draw_list->AddText(text_pos, color_bg, label_text);
...
#include <windows.h>
UINT ImGuiKeyToVKey(ImGuiKey key)
{
    switch (key)
    {
    ...
    case ImGuiKey_F12:           return VK_F12;
        ...
        // Add more ImGuiKey cases if necessary
    default:                     return 0; // Invalid key
    }
}

bool ImGui::Hotkey(const char* label, int* windows_key, int* imgui_key)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Determine current key string
    char key_buf[64] = "None";
    ImFormatString(key_buf, IM_ARRAYSIZE(key_buf), "%s", GetKeyName((ImGuiKey)*imgui_key));

    // Capture state
    static ImGuiID active_hotkey = 0;
    const bool is_active = (active_hotkey == id);
    if (is_active)
        strcpy(key_buf, "Press a key...");

    ImVec2 text_size = CalcTextSize(key_buf);
    ImVec2 button_size = ImVec2(text_size.x + g.Style.FramePadding.x * 2.0f, frame_height);

    // Layout
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 button_pos = pos;
    ImVec2 button_max = button_pos + button_size;
    ImRect button_bb(button_pos, button_max);
    ImVec2 total_size = ImVec2(button_size.x + spacing + label_size.x, frame_height);
    ItemSize(total_size, g.Style.FramePadding.y);
    if (!ItemAdd(button_bb, id))
        return false;
    ...
    if (is_active)
    {
        ...
        else if (IsKeyPressed(ImGuiKey_Backspace, false))
        {
            *imgui_key = 0;
            *windows_key = 0;
            active_hotkey = 0;
            ...
            {
                if (IsKeyPressed((ImGuiKey)i, false))
                {
                    *imgui_key = i;
                    *windows_key = ImGuiKeyToVKey((ImGuiKey)i);
                    active_hotkey = 0;
                    break;
                }
            }
    ...
    // Draw button
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(button_bb, id);
    RenderFrame(button_bb.Min, button_bb.Max, col, true);

    // ✨ Center text manually (pixel-perfect)
    ImVec2 center = ImVec2(
        button_bb.Min.x + (button_bb.GetWidth() - text_size.x) * 0.5f,
        button_bb.Min.y + (button_bb.GetHeight() - text_size.y) * 0.5f
    );
    RenderText(center, key_buf);

    // Draw label
    if (label_size.x > 0.0f)
        RenderText(ImVec2(button_bb.Max.x + spacing, button_bb.Min.y + g.Style.FramePadding.y), label);

    return pressed;
}

bool ImGui::ColorPreviewButton(const char* label, ImVec4* color, bool* rainbow)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float square_sz = GetFrameHeight();
    const ImVec2 button_pos = window->DC.CursorPos;
    const ImVec2 button_size = ImVec2(square_sz, square_sz);
    const ImRect button_bb(button_pos, button_pos + button_size);

    ItemSize(button_size, g.Style.FramePadding.y);
    if (!ItemAdd(button_bb, id))
        return false;

    bool hovered, held;
    bool pressed = ButtonBehavior(button_bb, id, &hovered, &held);
    const ImU32 col_frame = GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(button_bb, id);
    RenderFrame(button_bb.Min, button_bb.Max, col_frame, true);

    // Draw preview
    ImU32 color_u32 = ColorConvertFloat4ToU32(*color);
    const float pad = 2.0f;
    window->DrawList->AddRectFilled(button_bb.Min + ImVec2(pad, pad), button_bb.Max - ImVec2(pad, pad), color_u32, g.Style.FrameRounding);

    // Label
    if (label_size.x > 0.0f)
        RenderText(ImVec2(button_bb.Max.x + g.Style.ItemInnerSpacing.x, button_bb.Min.y + g.Style.FramePadding.y), label);

    // Open popup on click
    bool value_changed = false;
    if (pressed)
        OpenPopupEx(id, ImGuiPopupFlags_MouseButtonLeft);

    if (BeginPopupEx(id, ImGuiWindowFlags_AlwaysAutoResize))
    {
        if (ColorPicker4("##picker", (float*)color, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_DisplayHSV))
            value_changed = true;

        if (rainbow)
        {
            Separator();
            value_changed |= Checkbox("Rainbow", rainbow);
        }
